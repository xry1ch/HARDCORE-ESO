...
local Rule = {
    id = ID,
    title = "Swim Discipline: periodic water survival check",
    icon = ICON_SWIM,
...
local function UpdateHud()
    ...
    if sv.graceActive then
        ...
        statusLabel:SetText("Swim now " .. tostring(zo_floor(sv.swimProgressMs / 1000)) .. "/10s")
        ...
        timeLabel:SetColor(1, 0.96, 0.80, 1)
        if swimming then
            statusLabel:SetText("Swimming " .. tostring(zo_floor(sv.swimProgressMs / 1000)) .. "/10s")
        else
            statusLabel:SetText("Swim due")
        end
        statusLabel:SetColor(0.78, 0.86, 0.88, 0.96)
        progressFill:SetCenterColor(0.16, 0.72, 0.95, 0.92)
        iconControl:SetColor(0.80, 0.96, 1, 1)
...
local function CompleteSwimRequirement(sv)
    ResetCycle(sv)
    Alert("HARDCORE: Swim requirement complete.", SOUNDS.QUEST_ACCEPTED)
    UpdateHud()
end
...
local function StartGrace(sv)
    ...
    sv.graceRemainingMs = GRACE_MS
    sv.warnedGrace = true
    Alert("HARDCORE: Swim now. Challenge fails in 20 seconds.", SOUNDS.DUEL_START)
end

local function FailForMissedSwim()
    if HARDCORE and HARDCORE.FailChallenge then
        HARDCORE.FailChallenge("HARDCORE: Challenge failed. You did not swim in time.")
    end
end
...
local function AdvanceSwimDiscipline()
    ...
    if sv.graceActive then
        ...
        elseif sv.intervalRemainingMs <= 60 * 1000 and not sv.warnedDue then
            sv.warnedDue = true
            Alert("HARDCORE: Swim requirement due in one minute.", SOUNDS.QUEST_OBJECTIVE_STARTED)
        end
    end
...
function HARDCORE.DebugSwimDisciplineStatus()
    local sv = GetSV()
    d("Swim Discipline active=" .. tostring(Rule.active) ..
        " swimming=" .. tostring(Rule._isSwimming or IsSwimming()) ..
...
function HARDCORE.DebugSwimDisciplineCommand(action, arg1)
    ...
    if action == "due" then
        ...
        sv.graceRemainingMs = GRACE_MS
        sv.lastUpdateMs = GetFrameTimeMilliseconds()
        Alert("HARDCORE: Swim grace countdown started.", SOUNDS.DUEL_START)
        UpdateHud()
        HARDCORE.DebugSwimDisciplineStatus()
        return
    end
    ...
    if action == "progress" then
        ...
        if not seconds then
            d("Usage: /hc debug swim progress <seconds>")
            return
        end
    ...
    if action == "reset" then
        Rule:ResetTimer()
        d("Swim Discipline: timer and swim progress reset.")
        return
    end
    ...
    if action == "hud" then
        ...
        d("Swim Discipline: HUD forced visible until the next scene/update refresh.")
        return
    end

    d("Unknown Swim Discipline debug action: " .. tostring(action))
end
